...
VU를 단계적으로 증가시키면서 RPS, 응답시간, 실패율이 어떻게 변하는지 확인한다.

### 고정 조건

- 대상 API: `GET /schedules/1/seats`
- 실행 시간: 30초
- Think time: 1초 (`sleep(1)`)
- SQL 로그: 비활성화
- AVAILABLE 좌석: 90개
- 각 테스트 전 smoke test 1회 실행

### 측정 결과

| VU | 요청 수 |    RPS | 평균(avg) | 중앙값(med) | p95 | 최대(max) | 실패율 |
|---:|---:|-------:|---:|---:|---:|---:|---:|
| 5  | 149  | 4.80/s | 31.42ms  | 28.06ms  |46.38ms  | 52.64ms |0.00%   |
| 10 |  300  | 9.714213/s       |26.54ms  |25.79ms  | 35.74ms |42.09ms   |0.00%   |
| 30 |900   |  29.157893/s      | 24.91ms | 22.58ms | 38.17ms | 87.83ms | 0.00%  |
| 50 | 1500  |     48.529582/s   |24.46ms  |20.87ms  | 36.84ms |137.08ms  | 0.00%  |

### 결과 분석

- VU 증가에 비례해 RPS가 증가했는가? ->
  - 표를 보면 확연하게 RPS가 증가하였음 
- 어느 VU부터 평균과 p95 응답시간이 크게 증가했는가? 
  - 신기하게도 평균은 줄어들고있고 p95도 줄어드는 추세를 보임 
- RPS 증가가 둔화되거나 멈춘 지점이 있는가?
  - RPS은 어디있는지 파악이되지는않음 
- HTTP 요청 실패가 발생한 지점이 있는가?
  -  실패한지점은 없음 
- 서버가 안정적으로 처리할 수 있는 범위는 어디까지인가?
  - 불안정적인 부분은ㅇ벗음 
