...
현재 위치는 **3주차 Day 13 진입 단계**다.

완료하거나 구현을 시작한 내용:

- 요구사항, ERD, API 초안
- Member, Event, Schedule, Seat 기본 API
- ScheduleSeat 일괄 생성 및 전체 조회
- SeatHold Entity, Repository, Service
- `AVAILABLE → HELD` 상태 변경
- 서버에서 `expiresAt` 계산
- 만료 선점을 조회하는 스케줄러 구현 시작

현재 우선순위:

1. `ScheduleSeat.hold()`, `release()`, `confirm()`으로 상태 규칙 모으기
2. SeatHold 응답 DTO와 Controller 만들기
3. Postman으로 선점 및 만료 흐름 확인하기
4. 가짜 Payment와 Reservation으로 예매 흐름 완성하기
